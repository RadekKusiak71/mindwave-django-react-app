...
class CommentsViewSet(viewsets.ViewSet):
    ...
    def retrieve(self, request, pk=None):
        try:
            ...
            return Response({'detail': 'Comment not found'}, status=status.HTTP_204_NO_CONTENT)
        except:
...
class FriendsRequestsViewSet(viewsets.ViewSet):
    ...
    def retrieve(self, request, pk=None):
        try:
            ...
            return Response({'detail': 'No user were found'}, status=status.HTTP_204_NO_CONTENT)
        except:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def check_if_exists(self, request):
        ...
        try:
            ...
            if request.exists():
                serialized = FriendRequestSerializer(
                    request.first(), many=False)
    ...
    def accept_friend_request(self, request):
        ...
        try:
            friend_request: FriendRequest = FriendRequest.objects.get(
                id=data['request_id'])
            receiver = Profile.objects.get(id=friend_request.receiver.id)
            sender = Profile.objects.get(id=friend_request.sender.id)
...
class PostViewSet(viewsets.ViewSet):
    ...
    def get_daily_posts(self, request):
        ...
        todays_posts = Post.objects.filter(
            profile__friends=active_user_profile, created_date__date=set_date)
        ...
        serializer_data = PostSerializer(
            todays_posts, many=True, context=serializer_context).data
